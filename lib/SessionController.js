	...
	self.open = function(filename) {
		...
		var bind = function() {
			...
			self.dbus.on('ClientAppeared', function() {
				...
				if (process.send) process.send({ info: ['Client `' + clientName + '` came online'] });
			...
			self.dbus.on('ClientDisappeared', function() {
				...
				if (process.send) process.send({ info: ['Client `' + clientName + '` went offline'] });
			...
			self.dbus.on('PortAppeared', function() {
				...
				if (process.send) process.send({ info: ['Port `' + clientName + ':' + portName + '` came online'] });
			...
			self.dbus.on('PortDisappeared', function() {
				...
				if (process.send) process.send({ info: ['Port `' + clientName + ':' + portName + '` went offline'] });
			...
			self.dbus.on('PortsConnected', function() {
				...
				if (process.send) process.send({ info: ['Port `' + fromClientName + ':' + fromPortName + '` connected to `' + toClientName + ':' + toPortName + '`'] });
			...
			self.dbus.on('PortsDisconnected', function() {
				...
				if (process.send) process.send({ info: ['Port `' + fromClientName + ':' + fromPortName + '` disconnected from `' + toClientName + ':' + toPortName + '`'] });
			...
			iface.StartServer(function() {
				connect(function() {
					var sandbox = sanity.createSandbox({
						error:		function() {
							if (process.send) process.send({ error: Array.prototype.slice.call(arguments) });
						},
						warn:		function() {
							if (process.send) process.send({ warn: Array.prototype.slice.call(arguments) });
						},
						log:		function() {
							if (process.send) process.send({ log: Array.prototype.slice.call(arguments) });
						},
						info:		function() {
							if (process.send) process.send({ info: Array.prototype.slice.call(arguments) });
						},
						session:	session
					});

					if (filename) sandbox.require(filename);
	...
	self.matchPortDescriptors = function(output, input, callback) {
		...
		for (var inIndex in inPorts) {
			...
			for (var outIndex in outPorts) {
				...
				if (
					inPort.isInput && outPort.isOutput
