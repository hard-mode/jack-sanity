...
var ClientController = function(session, clientName, portName) {
	...
	self.findPortDescriptors = function() {
		if (!self.ports.length) {
			self.ports = session.findPortDescriptors(self.clientName, self.portName)
		}
...
module.exports.createClientController = function(session, clientName, portName) {
	return new ClientController(session, clientName, portName);
};